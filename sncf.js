...
/* ── TOPBAR ── */
.topbar{height:52px;background:var(--panel);border-bottom:1px solid var(--border);display:flex;align-items:center;padding:0 16px;gap:16px;flex-shrink:0;z-index:20;overflow:visible}
...
.search-wrap{flex:1;max-width:340px;position:relative;overflow:visible}
...
.sugg{position:absolute;top:calc(100% + 5px);left:0;right:0;z-index:9999;background:#0a1525;border:1px solid var(--border2);border-radius:10px;box-shadow:0 20px 60px rgba(0,0,0,.9);overflow:hidden;display:none}
...
async function loadDisruptions(){
  ...
  try{
    ...
    html+=list.map(function(d,i){
      ...
      var cause=d.cause||"";
      var msgs=d.messages||[];var msg="";for(var mi=0;mi<msgs.length;mi++){if(msgs[mi]&&(msgs[mi].text||msgs[mi].value)){msg=msgs[mi].text||msgs[mi].value;break;}}
